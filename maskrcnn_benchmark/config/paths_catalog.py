...
class DatasetCatalog(object):
    ...
    DATASETS = {
        ...
        "VG_stanford_filtered_with_attribute": {
            ...
            "image_file": "vg/image_data.json",
            "window_file": "vg/VG-sampled-windows.h5"
        },
        "openimage_v4": {
            ...
            "window_file": "openimages/open_image_v4/OIv4-sampled-windows.h5"
        },
        ...
        "openimage_v6": {
            ...
            "window_file": "openimages/open_image_v6/OIv6-sampled-windows.h5",
        },
        "VRD": {
            "img_dir": "vrd/sg_dataset/sg_%s_images",
            "roidb_file": "vrd/sg_annotations/%s_sgs.json",
            "dict_file": "vrd/sg_annotations/labels.json",
            "image_file": "vrd/sg_annotations/%s_images.json",
            "window_file": "vrd/sg_annotations/vrd-sampled-windows.h5"
        },
        "VGVRDUnion": {
            "img_dir": ["vg/VG_100K", "vrd/sg_dataset/sg_%s_images"],
            "roidb_file": ["vg/VG-SGG-with-attri.h5", "vgvrd/vrd/%s_sgs.json"],
            "dict_file": "vgvrd/vgvrd-dicts.json",
            "image_file": ["vg/image_data.json", "vgvrd/vrd/%s_images.json"],
            "window_file": ["vg/VG-sampled-windows.h5", "vgvrd/vrd/vrd-sampled-windows.h5"]
        }
    }
    ...
    def get(name, cfg):
        if "coco" in name:
            ...
            for k, v in args.items():
                if isinstance(v, list):
                    for i in range(len(args[k])):
                        args[k][i] = os.path.join(data_dir, v[i])
                else:
                    args[k] = os.path.join(data_dir, v)
            args['split'] = split
            args['img_dir'][1] = args['img_dir'][1] % "train" if split == "train" else args['img_dir'][1] % "test"
            args['roidb_file'][1] = args['roidb_file'][1] % "train" if split == "train" else args['roidb_file'][1] % "test"
            args['image_file'][1] = args['image_file'][1] % "train" if split == "train" else args['image_file'][1] % "test"
            ...
            args['roidb_file'] = args['roidb_file'] % split
            args['image_file'] = args['image_file'] % split
            # IF MODEL.RELATION_ON is True, filter images with empty rels
            # else set filter to False, because we need all images for pretraining detector
            args['filter_non_overlap'] = (not cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX) and cfg.MODEL.RELATION_ON and cfg.MODEL.ROI_RELATION_HEAD.REQUIRE_BOX_OVERLAP
            args['filter_empty_rels'] = True  #cfg.MODEL.RELATION_ON
            args['flip_aug'] = cfg.MODEL.FLIP_AUG
            args['custom_eval'] = cfg.TEST.CUSTUM_EVAL
            args['custom_path'] = cfg.TEST.CUSTUM_PATH
            return dict(
