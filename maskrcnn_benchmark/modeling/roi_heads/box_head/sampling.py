...
class FastRCNNSampling(object):
    ...
    def assign_label_to_proposals(self, proposals, targets):
        for img_idx, (target, proposal) in enumerate(zip(targets, proposals)):
            ...
            matched_idxs = self.proposal_matcher(match_quality_matrix)
            # Fast RCNN only need "labels" field for selecting the targets
            #target = target.copy_with_fields(["labels", "attributes", "img_lvl_obj_label", "img_lvl_rel_label"])
            target = target.copy_with_fields(["labels", "attributes"])
            matched_targets = target[matched_idxs.clamp(min=0)]
            ...
            #img_lvl_obj_label = matched_targets.get_field("img_lvl_obj_label").to(dtype=torch.float32)
            #img_lvl_rel_label = matched_targets.get_field("img_lvl_rel_label").to(dtype=torch.float32)
            ...
            #proposals[img_idx].add_field("img_lvl_obj_label", img_lvl_obj_label, is_global=True)
            #proposals[img_idx].add_field("img_lvl_rel_label", img_lvl_rel_label, is_global=True)
        return proposals
