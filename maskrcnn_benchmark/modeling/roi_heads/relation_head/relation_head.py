...
from .roi_contrastors import make_roi_contrastor
from .sampling import make_roi_relation_samp_processor
from ..window_head.window_head import build_roi_window_head
...
class ROIRelationHead(torch.nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        # mtl window
        if cfg.MODEL.MTL_WINDOW:
            self.window_head = build_roi_window_head(cfg, in_channels)   
    ...
    def forward(self, features, proposals, targets=None, logger=None, iteration=None):
        ...
        # mtl window
        if self.cfg.MODEL.MTL_WINDOW:
            # loss window for training the auxiliary tasks, the relation_logits_for_refined is for relation refinement
            relation_logits_for_refined, loss_window = self.window_head(features, proposals, refine_logits,
                                                                        relation_logits, rel_pair_idxs,
                                                                        targets, logger)
            output_losses.update(loss_window)
            num = [len(r) for r in rel_pair_idxs]
           
            relation_logits = torch.cat(relation_logits, 0) + relation_logits_for_refined.detach()
           
            relation_logits = relation_logits.split(num)

        # for test
        if not self.training:
            if self.cfg.MODEL.MTL_WINDOW:
                num = [len(r) for r in rel_pair_idxs]
                relation_logits_for_refined = relation_logits_for_refined.split(num)
            else:
                relation_logits_for_refined = None
            result = self.post_processor((relation_logits, refine_logits), rel_pair_idxs, proposals,
                                         relation_logits_for_refined=relation_logits_for_refined)
        ...
        if self.cfg.MODEL.ATTRIBUTE_ON and isinstance(loss_refine, (list, tuple)):
            output_losses.update(dict(loss_rel=loss_relation, loss_refine_obj=loss_refine[0], loss_refine_att=loss_refine[1]))
        else:
            output_losses.update(dict(loss_rel=loss_relation, loss_refine_obj=loss_refine))
