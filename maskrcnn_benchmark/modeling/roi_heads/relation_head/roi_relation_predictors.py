...
from .model_plain import PlainContext
from .utils_relation import layer_init, get_box_info, get_box_pair_info
from maskrcnn_benchmark.data import get_dataset_statistics

@registry.ROI_RELATION_PREDICTOR.register("PlainPredictor")
class PlainPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.use_bias = True
        self.use_objcontext = True
        assert in_channels is not None

        self.context_layer = PlainContext(config, self.num_obj_cls, self.num_rel_cls, in_channels)

        # post decoding
        self.hidden_dim = config.MODEL.ROI_RELATION_HEAD.CONTEXT_HIDDEN_DIM
        self.pooling_dim = config.MODEL.ROI_RELATION_HEAD.CONTEXT_POOLING_DIM  # 4096
        if not self.use_objcontext:
            self.edge_unary = nn.Linear(self.pooling_dim, self.hidden_dim)
            self.rel_compress = nn.Linear(self.hidden_dim, self.num_rel_cls, bias=True)
        else:
            self.post_emb = nn.Linear(self.hidden_dim, self.hidden_dim * 2)
            self.post_cat = nn.Linear(self.hidden_dim * 2, self.pooling_dim)
            if self.pooling_dim != config.MODEL.ROI_BOX_HEAD.MLP_HEAD_DIM:  # 4096
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        obj_dists, obj_rep = self.context_layer(roi_features, proposals, rel_pair_idxs, logger)
        ...
        if not self.use_objcontext:
            rel_dists = self.rel_compress(F.relu(self.edge_unary(union_features)))
        else:
            # post decode
            edge_rep = self.post_emb(obj_rep)
...
class MotifPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        if config.MODEL.ROI_BOX_HEAD.NUM_NEW_CLASSES > 0:
            self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_NEW_CLASSES  # for joint training on VG and VRD
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
