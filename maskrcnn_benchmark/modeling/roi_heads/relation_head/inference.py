...
class PostProcessor(nn.Module):
    ...
    def forward(self, x, rel_pair_idxs, boxes, relation_logits_for_refined=None):
        """
        Arguments:
        ...
        if relation_logits_for_refined is None:
            relation_logits_for_refined = [None] * len(relation_logits)
        ...
        results = []
        for i, (rel_logit, obj_logit, rel_pair_idx, box, rel_logit_for_refined) in enumerate(zip(
            relation_logits, finetune_obj_logits, rel_pair_idxs, boxes, relation_logits_for_refined
        )):
            if self.attribute_on:
            ...
            if cfg.OUTPUT_SAVE_REL_LOGIT:
                rel_logit = rel_logit[sorting_idx]
                boxlist.add_field('rel_logit', rel_logit)
                if rel_logit_for_refined is not None:
                    rel_logit_for_refined = rel_logit_for_refined[sorting_idx]
                    boxlist.add_field('rel_logit_for_refined', rel_logit_for_refined)
