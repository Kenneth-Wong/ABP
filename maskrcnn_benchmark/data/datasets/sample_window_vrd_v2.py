...
from maskrcnn_benchmark.data.datasets.vrd import load_graphs, load_image_infos
...
def main():
    ...
    parser.add_argument('--num_classes', default=101)
    ...
    # root = "datasets/vrd/"
    # ann_file = os.path.join(root, "sg_annotations/train_sgs.json")
    # image_file = os.path.join(root, "sg_annotations/train_images.json")
    # args.h5_output = os.path.join(root, "sg_annotations", args.h5_output)

    # for vgvrd/vrd
    root = "datasets/vgvrd/vrd/"
    ann_file = os.path.join(root, "train_sgs.json")
    image_file = os.path.join(root, "train_images.json")
    args.h5_output = os.path.join(root, args.h5_output)

    img_ids, boxes, gt_classes, relationships = load_graphs(ann_file, -1,
                                                            filter_empty_rels=True,
                                                            filter_non_overlap=False)
    img_infos = load_image_infos(image_file)
    img_info = [img_infos[id] for id in img_ids]
    # check
