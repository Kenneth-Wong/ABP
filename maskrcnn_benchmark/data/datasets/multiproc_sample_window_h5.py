...
def single_process(i, head_bbox, tail_bbox, labels, num_classes, min_size, window_num, max_rel_num):
    w, h = head_bbox.size
    ...
    while windows.bbox.size(0) < window_num:
        ...
        for i in range(len(tmp_windows)):
            ...
            head_bbox = head_bbox[indices]
            tail_bbox = tail_bbox[indices]
            ...
            all_head_bboxes = cat_boxlist([all_head_bboxes, head_bbox])
            all_tail_bboxes = cat_boxlist([all_tail_bboxes, tail_bbox])
...
def multiproc_sample_window(dataset, num_classes, logger):
    ...
    if os.path.exists(sampled_windows_path):
        ...
        for i in tqdm(range(len(dataset))):
            ...
            all_windows.append(
                pool.apply_async(single_process, (target, num_classes, min_size, window_num, max_rel_num)))
        pool.close()
        pool.join()
...
def main():
    ...
    parser.add_argument('--h5_output',
                        default='datasets/vg/VG-sampled-windows.h5',
    ...
    pool = Pool(4)
    collected_windows = []
    collected_rels = [[] for i in range(len(split_mask))]
    ...
    for i in tqdm(range(len(split_mask))):
        ...
        collected_rels[i] = np.column_stack((gt_classes_i[rels[:, 0]][:, None],
                                             gt_classes_i[rels[:, 1]][:, None],
                                             rels[:, -1][:, None]))

        collected_windows.append(pool.apply_async(single_process, (i, head_bbox, tail_bbox, labels,
                                                                   args.num_classes, args.min_size,
                                                                    args.window_num, args.max_rel_num)))

    pool.close()
    pool.join()
    collected_windows = [r.get() for r in collected_windows]

    print('generating dataset...\n')
    for windows_i in tqdm(collected_windows):
        i = windows_i.get_field("index")
        if len(windows_i) == 0:
            continue

        im_to_first_all_relation[i] = len(all_relation_labels)
        all_relation_labels = np.vstack((all_relation_labels, collected_rels[i]))
        ...
        all_clipped_head_bbox = np.vstack(
            (all_clipped_head_bbox, windows_i.get_field("clipped_head_bbox").bbox.numpy()))
        all_clipped_tail_bbox = np.vstack(
            (all_clipped_tail_bbox, windows_i.get_field("clipped_tail_bbox").bbox.numpy()))
