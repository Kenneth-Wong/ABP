...
def single_process(head_bbox, tail_bbox, obj_bbox, labels, num_classes, min_size, window_num):
    ...
    window_object_labels = torch.zeros(0, num_classes).float()
    ...
    while windows.bbox.size(0) < window_num:
        ...
        for i in range(len(tmp_windows)):
            ...
            # compute soft label
            obj_clipped = boxlist_clip_and_change_coordinate_to_window(obj_bbox, obj_bbox, window)
            clipped_obj_bboxes, _, norm_clipped_obj_bboxes, _, obj_indices = obj_clipped
            obj_label_win = labels[obj_indices]

            area_total = boxlist_union_area(norm_clipped_obj_bboxes)

            label_bg = np.sqrt(max(0., 1. - area_total))
            if label_bg == 1.:
                continue

            object_label = torch.zeros(num_classes)
            object_label[0] = label_bg

            # using boxes of each predicate
            for obj_cls in torch.unique(obj_label_win):
                cls_indices = torch.nonzero(obj_label_win == obj_cls).view(-1)

                cls_norm_clipped_obj_bboxes = norm_clipped_obj_bboxes[cls_indices]
                area_total = boxlist_union_area(cls_norm_clipped_obj_bboxes)
                object_label[obj_cls] = np.sqrt(area_total)

            object_label = object_label / torch.sum(object_label)
            #####################
            #object_label = torch.zeros(num_classes)

            keep_window_idx.append(i)
            window_object_labels = torch.cat((window_object_labels, object_label.view(1, -1)), 0)
    ...
    if num_all > window_num: # IMPORTANT!!!!: if num_all == window_num, the following codes not work!!!
        ...
        window_object_labels = window_object_labels[:window_num]
    ...
    windows.add_field("object_labels", window_object_labels)
...
def main():
    ...
    parser.add_argument('--num_predicates', default=51)
    ...
    for i in tqdm(range(len(split_mask))):
        ...
        all_object_labels.append(windows_i.get_field("object_labels").numpy())
    ...
    f.create_dataset("object_label", data=np.vstack(all_object_labels).astype(np.float32))
