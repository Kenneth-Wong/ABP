...
from maskrcnn_benchmark.data.datasets.visual_genome import resampling_dict_generation, \
...
class VRDDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
        ...
        # for debug
        if cfg.DEBUG:
            num_im = 400 if split == 'train' else 100
        ...
        self.ind_to_classes, self.ind_to_predicates = load_info(dict_file)  # contiguous 101, 71 containing __background__
        ...
        if self.custom_eval:
            ...
            img_ids, self.gt_boxes, self.gt_classes, self.relationships = load_graphs(
                self.roidb_file, num_im,
                filter_empty_rels=filter_empty_rels,
                filter_non_overlap=self.filter_non_overlap,
            )

            img_infos = load_image_infos(image_file)
            self.img_info = [img_infos[id] for id in img_ids]
            self.filenames = [os.path.join(img_dir, img_info['filename']) for img_info in self.img_info]
    ...
    def __getitem__(self, index):
        ...
        if img.size[0] != self.img_info[index]['width'] or img.size[1] != self.img_info[index]['height']:
            ...
            self.img_info[index]['width'] = img.size[0]
            self.img_info[index]['height'] = img.size[1]
    ...
    def get_statistics(self):
        fg_matrix, bg_matrix, rel_counter_init, cooccur_matrix, occur_num = get_VRD_statistics(img_dir=self.img_dir,
...
def get_VRD_statistics(img_dir, roidb_file, dict_file, image_file, must_overlap=True, window_file=None):
    train_data = VRDDataset(split='train', img_dir=img_dir, roidb_file=roidb_file,
                           dict_file=dict_file, image_file=image_file,
                           filter_duplicate_rels=True, filter_non_overlap=False, window_file=window_file)
...
def load_image_infos(image_file):
    """
...
def load_graphs(ann_file, num_im, filter_empty_rels, filter_non_overlap):
    ...
    for i, ann in enumerate(anns):
        ...
        # 3 images are removed here, 3780 - 3 = 3777
        if len(rels) == 0:
            assert filter_empty_rels
            continue  # filter

        # if is true: 3777 -> 3698
