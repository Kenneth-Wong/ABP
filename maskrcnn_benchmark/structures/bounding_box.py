...
class BoxList(object):
    ...
    def __init__(self, bbox, image_size, mode="xyxy"):
        ...
        #self.global_fields = [] # e.g. img_lvl field, which is not the same size as object bboxes and should not respond to __getitem__ slicing v[item]

    def add_field(self, field, field_data, is_triplet=False, is_global=False):
    ...
    def resize(self, size, *args, **kwargs):
        ...
        if ratios[0] == ratios[1]:
            ...
            for k, v in self.extra_fields.items():
                ...
                # elif k in self.global_fields:
                #     bbox.add_field(k, v, is_global=True)
                else:
                    bbox.add_field(k, v)
            return bbox
    ...
    def __getitem__(self, item):
        ...
        for k, v in self.extra_fields.items():
            if not isinstance(v, torch.Tensor):  # directly add, not related to the bbox
                bbox.add_field(k, v)
            elif k in self.triplet_extra_fields:
    ...
    def copy_with_fields(self, fields, skip_missing=False):
        ...
        for field in fields:
            if self.has_field(field):
                ...
                # elif field in self.global_fields:
                #     bbox.add_field(field, self.get_field(field), is_global=True)
                else:
