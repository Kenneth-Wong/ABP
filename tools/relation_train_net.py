...
SHOW_COMP_GRAPH = True #False, some bugs for vctree?
...
def train(cfg, local_rank, distributed, logger):
    ...
    if cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR in ["IMPPredictor", "PlainPredictor"]:
        slow_heads = ["roi_heads.relation.box_feature_extractor",
                      "roi_heads.relation.union_feature_extractor.feature_extractor",]
    else:
        slow_heads = []

    #if cfg.MODEL.CONTRASTIVE_BRANCH.ENABLED:
    #    slow_heads += ["roi_heads.relation.contrastors"]
    ...
    for iteration, (images, targets, _) in enumerate(train_data_loader, start_iter):
        ...
        if not SHOW_COMP_GRAPH and get_rank() == 0:
            try:
                g = vis_graph.visual_computation_graph(losses, model.named_parameters(), cfg.OUTPUT_DIR, "total_loss-graph")
                g.render()
                for name, ls in loss_dict_reduced.items():
                    g = vis_graph.visual_computation_graph(losses, model.named_parameters(), cfg.OUTPUT_DIR, f"{name}-graph")
                    g.render()
            except:
        ...
        if iteration % cfg.SOLVER.PRINT_FREQ == 0 or iteration == max_iter:
            logger.info(
